...
1. "选中态看起来不像标签、像纯文字"是真问题，根因在我上一轮把自绘胶囊换成了 dsh 的 `Pill`：
   `Pill` 未选中时**没有外框**，选中态的填充（`button-ghost-active-fill`）又几乎等于对话框自身底色
   （实测未选中 `rgb(44,44,46)` / 选中 `rgb(67,69,74)`），于是"选中"反而更不像一个控件。改回自绘胶囊：
   **两态共用同一个 `--dsw-alias-border-l4` 外框**（这是用户第 3 轮就定的口径），选中只改填充
   （透明 → `--dsw-alias-bg-module-platform`，与行内标签同一个底色）与字重（400 → 600）。
   实测：未选中 `transparent` + 1px 外框 + 二级文字；选中 `rgb(53,54,56)` + 1px 外框 + 一级文字 + 600
   ——选中态与行内标签**完全同款**，且两态都看得出是个标签。
2. `套餐内` 与 `免费` 改为**互斥**：这不是口味问题，而是"两个都选只能得到空列表"——免费模型永不
   `planCovered`，两个维度不可能同时成立（构建门专门断言了这条语义）。互斥逻辑抽成纯函数
   `toggleFilter(flags, tag)`，选中一个即释放另一个；能力过滤片不受影响，仍可多选（AND）。
   实测：`套餐内` → 30/80，切到 `免费` → 13/80 且 `套餐内` 自动熄灭，再叠 `image`+`pdf` → 3/80。

### ✅ 模型目录的两个入口合成一个（2026-09-11）

第七轮反馈：「Kenari 模型目录」里点「添加模型」和点「获取可用模型」，功能应该做成一样的。

这两颗按钮都是 dsh 的 `ModelListEditor` 渲染的（`addModel: '添加模型'` / `fetchModels: '获取可用模型'`），
原生语义一个是"插一行空白条目让你手打 id"，一个是"问提供方要目录"。对 Kenari 来说目录才是事实来源，
手打 id 只会绕过它，所以两个标签一起接管，落到同一个对话框。

**接管范围一开始是错的 —— 这是这一轮真正的收获。** 原来的判定是"从按钮往上走，第一个含有标记的祖先
存在即接管"。实测 DOM 是 `<ul>` 包着每张卡一个 `<li>`，标记在 Kenari 那张 `<li>` 里 —— 于是**从别的
provider 的按钮往上走，第一个含标记的祖先是那个 `<ul>`**，兄弟卡片的同名按钮也被一并接管。症状很隐蔽：
dsh 原生「添加模型」本该插一行空白条目，被接管后既不插行也不报错，只是弹出了插件的对话框 —— 点的人只会
觉得"这个按钮怎么怪怪的"。改成**从标记这一侧问**：取标记自己的卡片（`marker.closest('li')`，退路是它的
父元素），再用 `card.contains(button)` 判定。谁是卡片由标记定义，范围就是精确的；哪天 dsh 的行卡片换了
标签，`closest('li')` 落空、`contains` 为假，接管自动退回原生（fail-open），不会误伤。

实测（1280 视口，本机 3100 临时实例，用户自己的实例与 3099 全程未动）：

- Kenari 卡「Add model」→ 插件对话框（80 行 / 148 个标签 / 820px 宽），该卡模型行数不变（7 → 7）；
- Kenari 卡「Fetch available models」→ 同一个对话框（同 80 行 / 148 个标签）；
- DeepSeek 卡「Add model」（卡片无标记）→ **原生生效**：模型行 4 → 5、新增的 id 输入框为空，
  且没有弹出任何插件对话框。

构建门同步补了两条断言：标记定义的是它自己的 `<li>`（不是上面的 `<ul>`）；兄弟卡片的按钮判为 null。
门里的假 DOM 与实测结构同构 —— `<ul>` → 两张 `<li>`、标记藏在 slot 包一层的 `DIV` 里 ——
因为正是"少一层 / 多一层兄弟"这类偏差让这个 bug 逃过了上一版的门。

### ✅ 模型条目的显示名与输出上限（2026-09-11）

反馈：「设置 → 模型 → 自定义设置 → 模型目录」里，模型行①没有显示名称、②展开后的
「最大输出 token」是个固定的 32K。两处都是**插件写进路由的模型条目**缺字段，不是 dsh 的问题：

- 不写 `name` → dsh 的「显示名称」是空框；
- 不写 `maxTokens` → 整条路由共用 pi-ai 的 `defaultMaxTokens`（**32768**），每个模型展开看到的
  都是同一个数（那个 `32K` 其实还是输入框的 placeholder，值本身是空的）。

**两个字段都必须推**，因为目录里都没有（对全部 76 行实测，见 Kenari 知识库）：

- **显示名**：目录只有 8 个模型带 `name`，且都不是会话模型——61 个会话模型一个都没有。
  所以按 id 还原：`-` 分片、版本号合并成点分（`5-3` → `5.3`、`v2-5` → `v2.5`）、缩写与品牌查表
  （`gpt`→`GPT`、`deepseek`→`DeepSeek`、`mimo`→`MiMo`）、参数量后缀转大写（`120b`→`120B`、`a55b`→`A55B`）。
  **校验用的是目录自己的事实**：把 `name` 抹掉再推一遍，Kenari 自己写了名字的那 8 个模型
  8/8 与厂商写法逐字一致（`veo-3.1-lite` → `Veo 3.1 Lite`、`minimax-speech-2-8-turbo` →
  `MiniMax Speech 2.8 Turbo`…），`test/catalog-view.mjs` 断言的就是这条性质，目录换模型也成立。
- **输出上限**：目录没有这个字段，取**窗口的 1/4**，向下取整到 1K，上限 65536，且**不设下限**。
  1/4 是"一次回答最多占多少窗口"的常见取法（厂商公开的比例在 6%–32% 之间，取偏宽的一侧，
  因为写代码时"被截断"比"留得多"更常见）；不设下限是因为下限会算错小窗口——早期版本用 8192 兜底，
  而目录里 `bge-m3` 的窗口只有 8192，于是推荐值等于**整个窗口**，这条 bug 是测试里
  「推荐值 ≤ 窗口 1/4」的性质断言抓到的。目录不公布窗口的 18 个模型（只有 `qwen3-8-max` 是会话模型）
  不写该字段：没有窗口可依据时任何数都是编的。
- **两条实测支撑**（都写进了 Kenari 知识库）：网关**不校验** `max_tokens`（4096 / 65536 / 262144
  全部 200），所以上限只是"别写没有意义的数"；dsh 侧 `maxTokens` 只当请求输出上限用，
  **不会**从 `contextWindow` 里扣，所以调大它不挤压输入预算。

同期另一条反馈：**`embedding` 过滤片独占**。它标的是另一个群体（embedding 模型没有 chat 端点），
和任何其他维度同时选都只会得到空列表（AND 语义），所以选它清掉其他、选其他清掉它——
与 `套餐内`/`免费` 的互斥同一个理由，只是作用域是全部维度。抽成同一个纯函数 `toggleFilter`：
`EXCLUSIVE_FILTERS` 管两两互斥，`SOLO_FILTERS` 管独占。

实测（3100，settings.yaml 未被写入）：四个兜底模型的显示名称分别为 `DeepSeek v4 Flash` /
`GLM 5.3 Flash` / `GPT 5.6 Luna` / `MiMo v2.5`；展开第 1 行「Max output tokens」的值是
`65536`（不再空着）；目录 80 个 profile 全部通过真实 pi-ai schema；`displayNameOf` 与目录
自带的 8 个名字逐字一致；`embedding` 与 `image` 互清（4/80 ↔ 46/80）。
