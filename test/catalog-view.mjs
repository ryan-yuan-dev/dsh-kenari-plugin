...
// 是 slug→模型名 规则的**标准答案**：把 name 抹掉再推一遍，必须与厂商写法逐字一致。
// 这条比断言某个具体模型强：它验证的是规则本身，且目录换了模型也照样成立。
//
// 这里读的是**原始目录**而不是视图：视图的 `name` 已经过同一条规则，拿它当"厂商写法"
// 会把这条断言变成自证。
...
// 两个口径是**故意**分开的：插件自己那一页的模型表用模型自己的名字（整页都在 Kenari 节里，
// 每行再写一遍 Kenari 是噪音），只有写进路由的 profile.name 带 `Kenari ` 前缀——那才是 dsh
// 的模型按钮、会话头部与 `/` 菜单印出来的字符串，不带前缀就分不出 Kenari 与官方 DeepSeek 的
// 同名模型。对照项取自原始目录行，不从视图反推。
const rawById = new Map(rawModels.map((m) => [m.id, m]))
check('插件自己的模型表用模型自己的名字，不带路由前缀',
  view.models.every((m) => m.name === displayNameOf(rawById.get(m.id) ?? { id: m.id })),
  `${view.models.length} 个，例如 ${view.models[0] === undefined ? '—' : `${view.models[0].id}→${view.models[0].name}`}`)
check('写进路由的 profile.name 恰好多一个前缀（不多不少）',
  view.models.every((m) => m.profile.name === DISPLAY_NAME_PREFIX + m.name),
  `${view.models.length} 个，例如 ${view.models[0] === undefined ? '—' : `${view.models[0].id}→${view.models[0].profile.name}`}`)

// 视图的 name 必须经过派生，而不是目录原样透传：后者对无名模型等于 id，界面把 id 印两遍。
const unnamed = view.models.filter((m) => (rawNameById.get(m.id) ?? '') === '')
check('目录没给 name 的模型拿到模型名而不是 id 复读',
...
// 反过来说：目录自己给了 name 的，视图必须照抄厂商写法（前缀不在这一层），不能被 id 还原覆盖掉。
