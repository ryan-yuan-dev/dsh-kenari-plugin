...
这个插件在 **ZCode** 里写的，会话模型主要用 opencode 的 DeepSeek V4.1 Flash 和 Kenari 的 `deepseek-v4-flash`。原因很实际：写这东西要反复读 dsh 的源码并跨文件改，两个模型都扛得住长上下文，而 Kenari 按量计费，试错成本比按月订阅低。

如果你也打算用这套组合，下面两个是**推荐链接**：

- [Kenari](https://kenari.id/code/KNR-KKRNAJ)：本插件的 21 个工具都指向它，注册后拿到的返现和额度以官网当时活动为准。通过这个链接注册，我能拿到一点推荐奖励，这算是维护这个插件的一点回报。
- [opencode Go](https://opencode.ai/go?ref=343F5JW4RA)：$10/月，按 5 小时窗口给额度，我用它跑 dsh 会话模型。

用不用推荐链接不影响插件的任何功能，所有能力对所有账户一视同仁。
