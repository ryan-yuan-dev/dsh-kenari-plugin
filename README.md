...
- **显示名称**：目录只给 8 个模型写了 `name`，而且都不是会话模型（语音/图像/视频那些），
  ...
  它是纯展示字段（不参与请求），随时可以自己改。派生的名字是**唯一口径**：写进路由的
  `profile.name`、`/api/kenari.models` 的 `name`、设置页模型列表的「显示名称」列都是它。
  不这么做就会两头不一致——`/api/kenari.models` 曾经原样透传目录的 `name`，无名模型的
  `name` 于是等于 id，界面把 id 印两遍（列表的第二列就是从那儿来的；dsh 的 discovery 也会
  在端点没给名字时用 id 兜底，所以列表不能直接信它）。
...
每个新会话的智能标题默认带一个 `20260911174258-` 形式的前缀（**本会话第一条人类消息的时间**，
本机时区），方便在会话列表里按时间辨认。前缀由插件在服务端写进 `session/title` 事件，所以 Web、TUI、
headless 看到的是同一个字符串。

时间取第一条**人类**消息，而不是 `session.header.createdAt`：dsh 的 Web 侧会复用空白会话，记录可能
在工作区打开时就建好了，比真正开口早几个小时。没有任何人类消息时（例如对空会话手动改名）才退回
记录创建时间。
...
fork 子会话继承父标题时，前缀会换成**子会话自己的开始时间**（继承来的父会话历史不计；
浏览器 fork 之后 dsh 会再用一次 rename 把父标题带进子会话，这一步也一并处理）。
...
- **前缀模板没有转义**：模板里出现 `yyyy`、`MM` 这类子串一律按 token 替换。剥旧前缀时按
  「当前模板 + 对应会话的开始时间」精确重放，所以旧前缀能被干净剥掉；只有一种罕见情形会留下
...
| 模型 | 当前路由可用的模型，三列：**模型 ID / 显示名称 / 上下文**。默认只列前 3 个，其余用「展开全部 N 个」打开——提供方自述的目录有几十条，一屏铺满没法看 |
