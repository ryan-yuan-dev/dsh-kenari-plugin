...
- 每行只显示**模型 id**（不显示厂商给的可读名：id 才是请求与路由条目里用的那个字符串），
  后面跟着**能力标签**（`image` / `audio` / `video` / `pdf` / `embedding`）、`免费`，
  付费模型还会带一个 `套餐内` 标签——表示它在订阅套餐的覆盖范围内（免费模型不标注：
  它们本来就不花钱，覆盖与否没有意义）。**不显示套餐档位名**：这里只回答"订阅能不能用"
- 所有标签同一个样式（同样的字号、描边与圆角），不区分主次；标签永不压缩，
  一行放不下就整体换到下一行
